...
def crawl_yahoo_finance(keyword, days=14):
    """Yahoo Finance 뉴스를 크롤링합니다."""
    print(f"\n[Yahoo Finance] '{keyword}' 뉴스 수집 중...")
    ...
    try:
        ...
        search_url = f"https://finance.yahoo.com/quote/{keyword}/news"

        # 키워드가 회사명인 경우 ticker symbol로 변환 시도
        ticker_map = {
            'NVIDIA': 'NVDA',
            'Tesla': 'TSLA',
            'Apple': 'AAPL',
        ...
        ticker = ticker_map.get(keyword, keyword)
        ...
        if not news_items:
            # 대체 선택자 시도
            news_items = soup.find_all('div', {'data-test': 'news-item'})
        ...
        for item in news_items[:20]:  # 최대 20개
            try:
                ...
                link = 'https://finance.yahoo.com' + link_elem['href'] if link_elem else ''
                ...
                date_text = time_elem.get_text(strip=True) if time_elem else 'Recently'

                articles.append({
                    'title': title,
                    'content': content,
                    'source': 'Yahoo Finance',
                    'date': datetime.now(),  # Yahoo는 정확한 날짜 파싱이 어려움
                    'date_text': date_text,
                    'link': link
                })
...
def crawl_reuters(keyword, days=14):
    """Reuters 뉴스를 크롤링합니다."""
    print(f"\n[Reuters] '{keyword}' 뉴스 수집 중...")
    ...
    try:
        # Reuters 검색 URL
        search_url = f"https://www.reuters.com/site-search/?query={keyword}"
        ...
        # Reuters 뉴스 항목 찾기
        news_items = soup.find_all('li', class_='search-result-indiv')
...
def crawl_multisource_news(keyword, days=14):
    """멀티 소스에서 뉴스를 크롤링합니다."""
    ...
    # 미국 관련 키워드인 경우 영문 소스 우선
    if category in ['미국증시', '미국기업']:
        english_keyword = translate_keyword_for_english(keyword)

        # Yahoo Finance 크롤링
        yahoo_articles = crawl_yahoo_finance(english_keyword, days)
        all_articles.extend(yahoo_articles)

        # Reuters 크롤링
        reuters_articles = crawl_reuters(english_keyword, days)
...
def generate_enhanced_report(keyword, analyzed_articles, category, date_str=None):
    ...
    md_content = f"""# {icon} {keyword} 뉴스 분석 리포트 (Multi-Source)

**분석 일시**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
**분석 대상 날짜**: {date_str}
**수집 기간**: 최근 14일
**카테고리**: {category}
**데이터 소스**: Yahoo Finance, Reuters, Naver News
...
- **분석 방법**: Multi-Source 크롤링 + 키워드 기반 감성 분석
- **데이터 출처**: Yahoo Finance, Reuters, Naver News
- **분석 키워드 수**: 호재/악재 키워드 각각 약 70개 (영문 포함)
- **카테고리별 맞춤 분석**: 미국증시, 거시경제, 산업테마 등 카테고리별 특화 키워드 적용

---

*🤖 Generated with Claude Code (Multi-Source Version)*
...
def main():
    """메인 함수"""
    print("=" * 80)
    safe_print("📰 Multi-Source 뉴스 분석 도구 (Yahoo Finance + Reuters + Naver)")
    print("=" * 80)
    ...
    try:
        ...
        # 1. 멀티 소스 뉴스 크롤링
        articles = crawl_multisource_news(keyword, days=14)
