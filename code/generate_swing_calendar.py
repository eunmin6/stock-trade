...
def load_trading_results():
    ...
    for md_file in report_dir.glob('????-??-??.md'):
        ...
        try:
            ...
            # 거래 타입별 수익 현황에서 스윙투자 정보 추출
            if '스윙투자' in content:
                # "| 스윙투자 | 3건 | 883,340원 |" 형식 파싱
                for line in content.split('\n'):
                    if '스윙투자' in line and '|' in line:
                        parts = [p.strip() for p in line.split('|')]
                        if len(parts) >= 4 and parts[1] == '스윙투자':
        ...
        if '## 💰 거래별 손익 상세' in content:
            ...
            for line in lines:
                ...
                if in_table:
                    ...
                    # 데이터 행 파싱 (스윙투자만)
                    if line.startswith('|') and '스윙투자' in line:
                        parts = [p.strip() for p in line.split('|')]
                        if len(parts) >= 7:
                            try:
                                rank = parts[1]
...
def generate_report():
    ...
    print(f"\n총 {len(trading_results)}일의 스윙투자 데이터 로드 완료\n")

    if not trading_results:
        print("스윙투자 데이터가 없습니다.")
        return
...
"""
        ...
        # 파일 저장
        output_file = f"report/tradings/스윙투자_{year:04d}-{month:02d}.md"
